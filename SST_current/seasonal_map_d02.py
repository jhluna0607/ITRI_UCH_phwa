...
for sc in (scen):
    fnm = root+'itri_uch_avg_d02_'+sc+'_zlev.nc'
...
xtick = np.arange(119.53,119.57,.01)
ytick = np.arange(23.34,23.365,.005)
...
for y in range(2):
    ...
    for s in range(4):
        ...
        Q = ax[s].quiver(lon[::dl],lat[::dl],np.squeeze(us[s,::dl,::dl,y]),\
                         np.squeeze(vs[s,::dl,::dl,y]),scale=10,width=4e-3)
    ...
    plt.quiverkey(Q,1.15,-.02,.5,label='0.5 m/s',labelpos='S')
...
for s in range(4):
    ...
    Q = ax.quiver(lon[::dl],lat[::dl],np.squeeze(us[s,::dl,::dl,0]),\
                         np.squeeze(vs[s,::dl,::dl,0]),scale=10,width=4e-3)
    
    ax.clabel(cm, fontsize=10)
    rect = patches.Rectangle((np.min(lon),np.max(lat)-y),x,y,edgecolor='k',facecolor='w')
    currentaxis.add_patch(rect)
    plt.quiverkey(Q,.09,.95,.5,label='0.5 m/s',labelpos='S')
    #plt.quiverkey(Q,-.12,-.05,.5,label='0.5 m/s',labelpos='S', fontproperties={'size':8})
    ...
    ax.grid(zorder=0)
    #fig.savefig('seasonal_map_d02_mean_'+sea_mon[s]+'_qbox.png',bbox_inches='tight', transparent=True, dpi=330)
...
cbar = fig.colorbar(cn,cax=cbar_ax,orientation='vertical',extend='both',ticks=np.arange(20,30.1,2))
cbar.ax.set_title('\N{DEGREE SIGN}C')
fig.savefig('seasonal_map_d02_mean_cbar.png',bbox_inches='tight', transparent=True, dpi=330)
plt.show()
...
for s in range(4):
    ...
    plt.quiverkey(Q,-.12,-.05,.03,label='0.03 m/s',labelpos='S', fontproperties={'size':8})
    ...
    ax.set_xticklabels([f'{t:.2f}\N{DEGREE SIGN}E' for t in xtick])
    ax.grid(zorder=0)
    #fig.savefig('seasonal_map_d02_diff_'+sea_mon[s]+'.png',bbox_inches='tight', transparent=True, dpi=330)
    plt.show()
# Colorbar
fig,ax = plt.subplots(figsize=(1,3))
plt.axis('off')
...
cbar = fig.colorbar(cn,cax=cbar_ax,orientation='vertical',extend='both',ticks=np.arange(.5,1.6,.2))
cbar.ax.set_title('\N{DEGREE SIGN}C')
fig.savefig('seasonal_map_d02_diff_cbar.png',bbox_inches='tight', transparent=True, dpi=330)
plt.show()
