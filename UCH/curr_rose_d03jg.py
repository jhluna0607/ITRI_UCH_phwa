...
for sn, sc in enumerate(scen):
    ...
    for s, sea in enumerate(sea_mon):
        ...
        ax[x].bar(dir[keynm], mag[keynm], normed=True, opening=.9, edgecolor='white',\
                bins = np.arange(0,.75,.1), cmap=cm.YlGnBu)

        # Plot the Quiver
        Q = ax[x].quiver(0, 0, um[keynm], vm[keynm], scale=.32, color='black')
        if sn == 1:
            tmpu = um[f'ssp_{sea_sss[s]}'] - um[f'his_{sea_sss[s]}']
            tmpv = vm[f'ssp_{sea_sss[s]}'] - vm[f'his_{sea_sss[s]}']
            Qr = ax[x].quiver(0, 0, tmpu, tmpv, scale=.08, color='red')   
        else:   
            ax[x].set_title(sea_str[s], fontsize=16, fontweight=550, y=1.2)

        ax[x].set_yticks(np.arange(6,31,6))
        ax[x].set_yticklabels(['6%','12%','18%','24%','30%'], fontsize=8)
        ax[x].grid(linewidth=.2)
        x = x+1

plt.quiverkey(Q,1.48,.62,.04,label='0.04 m/s',labelpos='E')
plt.quiverkey(Qr,1.48,.52,.01,label='0.01 m/s',labelpos='E')
ax[3].legend(title='Current Speed (m/s)', bbox_to_anchor=(1.2,-.55))
ax[3].text(1.37, .45, 'Jiangjun', fontsize=14, fontweight='bold',transform=ax[3].transAxes)
...
for sn, sc in enumerate(scen):
    ...
    for s, sea in enumerate(sea_mon):
        ...
        # Plot the Quiver
        Q = ax.quiver(0, 0, um[keynm], vm[keynm], scale=.32, color='black')
        v = np.sqrt(um[keynm]**2+vm[keynm]**2)
        ax.text(270*np.pi/180, 10, f'{v[0]:.3f} m/s', fontsize=8)
        if sn == 1:
            tmpu = um[f'ssp_{sea_sss[s]}'] - um[f'his_{sea_sss[s]}']
            tmpv = vm[f'ssp_{sea_sss[s]}'] - vm[f'his_{sea_sss[s]}']
            v = np.sqrt(tmpu**2+tmpv**2)
            Qr = ax.quiver(0, 0, tmpu, tmpv, scale=.08, color='red')
            ax.text(155*np.pi/180, 15, f'{v[0]:.3f} m/s', fontsize=8, color='red') 
        ax.set_yticks(np.arange(6,31,6))
        ax.set_yticklabels(['6%','12%','18%','24%','30%'], fontsize=8)
        ax.grid(linewidth=.2)
        fig.savefig('wind_rose_d03jg_'+sc+'_'+sea_sss[s]+'.png', \
...
fig.savefig('wind_rose_d03jg_legend.png', \
                    bbox_inches='tight', transparent=True, dpi=330)
plt.show()